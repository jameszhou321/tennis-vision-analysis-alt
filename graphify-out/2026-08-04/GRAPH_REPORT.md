...
## Corpus Check
- 99 files · ~97,157 words
- Verdict: corpus is large enough that graph structure adds value.

## Summary
- 993 nodes · 1595 edges · 78 communities (66 shown, 12 thin omitted)
...
- MSTFormer Model Architecture
- Ablation & Component Configs
- Ablation Results Mapping
- Audio-Video Fusion Detection
- Config Reference & Figures
- Data Pipeline & Ball Tracking Docs
- YOLO Single-Frame Dataset
- Pose Feature Dataset Tools
- Court Detection & Homography
- Demo Inference Thread
- Demo Main Window (PyQt5)
- Batch Rally Pipeline
- Pose Tracker (Player Tracking)
...
- Trajectory Generation & Radar
- TrackNet Ball Tracker
- Forehand Frame Extraction
...
2. `MSTFormer` - 45 edges
3. `TennisActionDataset` - 38 edges
...
- `MSTFormer` --shares_data_with--> `parallel_backbones config toggle (run 3 backbones concurrently, VRAM-heavy)`  [EXTRACTED]
  src/model/mst/model_main.py → configs/CONFIG_REFERENCE.md
...
- `docs/architecture.md — Project File Manifest & Module Dependencies` --semantically_similar_to--> `src/README.md — Source Code Overview`  [INFERRED] [semantically similar]
  docs/architecture.md → src/README.md
- `Main Model Training Curve (85.37% Test Acc)` --references--> `train.py — MSTFormer training entry point`  [INFERRED]
  docs/figures/fig1_main_training_curve.png → src/model/mst/README.md
- `docs/architecture.md — Project File Manifest & Module Dependencies` --references--> `CourtDetector`  [EXTRACTED]
  docs/architecture.md → src/court_detector.py
...
### Community 0 - "MSTFormer Model Architecture"
Cohesion: 0.06
...
### Community 1 - "Ablation & Component Configs"
Cohesion: 0.08
Nodes (46): Class-imbalance handling in MSTFormer training — focal loss (gamma=2.0) + per-class weights [1.0,4.0,5.0,4.0,1.5] to offset forehand/backhand/serve underrepresentation vs idle/movement; compared against plain cross-entropy, merge_visual_tokens toggle — Perceiver-style resampling merges 3 visual streams into shared tokens vs independent per-stream tokens (rationale: trades sequence length/VRAM for potential cross-stream information sharing), use_player_crops toggle — enables/disables per-player cropped visual streams (rationale: isolate crop-stream contribution vs full-frame-only), use_pose toggle — enables/disables 125-dim pose feature token (rationale: isolate pose-stream contribution to action accuracy), use_visual toggle — disables all visual streams, isolating pure-pose performance (rationale: quantify overall visual contribution), visual_backbone selection (yolo11 default vs resnet18/vit/raw) — rationale: compare detection-pretrained vs general ImageNet-pretrained vs lightweight extraction, abl_global_only.yaml — Ablation: Full-frame Visual Only (no crops, no pose), abl_no_crops.yaml — Ablation: No Player Crops (full-frame visual + pose) (+38 more)

### Community 2 - "Ablation Results Mapping"
Cohesion: 0.08
Nodes (36): MSTFormer Action Recognition Model, MSTFormer Main Model: 85.37% Test Accuracy, Train/Test Accuracy Gap (Overfitting Signal, ~44 epochs), ablation/abl_global_only.yaml (use_pose=false + use_player_crops=false), ablation/abl_no_crops.yaml (use_player_crops=false), ablation/abl_no_pose.yaml (use_pose=false), ablation/abl_no_visual.yaml (use_visual=false, pose only), class_weights per-class loss weighting field (+28 more)

### Community 3 - "Audio-Video Fusion Detection"
Cohesion: 0.09
Nodes (28): _bandpass_filter(), compute_impact_score_series(), extract_audio_wav(), get_score_at(), RallyStateMachine, audio_video_fusion.py — Audio Impact Detection + Hysteresis State Machine…, WAITING / POINT_ACTIVE hysteresis state machine (per the fusion design doc's…, Feeds one new sample; returns the status string for this sample ("PLAYING… (+20 more)
...
### Community 5 - "Data Pipeline & Ball Tracking Docs"
Cohesion: 0.12
Nodes (31): BoT-SORT (player tracking algorithm used in offline pipeline), docs/architecture.md — Project File Manifest & Module Dependencies, data/rallies_annotated/ — Manually Annotated Rally Data, data/rallies_annotating/ — Annotation Workspace (_progress.json), data/rallies_new/ — Segmented Rally Clips (main.py output), data/rallies_train/ — MSTFormer Training Data, Three-Stage CV Pipeline (Court Detection → Pose Tracking → Action Recognition), docs/training_list.txt — Reference Video URL List (+23 more)

### Community 6 - "YOLO Single-Frame Dataset"
Cohesion: 0.08
Nodes (20): Action Class Taxonomy (idle/forehand/backhand/serve/movement), collect_samples(), create_datasets(), dataset.py — single-frame dataset (model/yolo), Dataset, YOLO Single-Frame Action Classification — Dataset Reads pre-extracted frames…, Splits the dataset into train/test sets by rally (consistent with mst training…, Creates training and testing datasets. (+12 more)

### Community 7 - "Pose Feature Dataset Tools"
Cohesion: 0.10
Nodes (23): 125-dim Pose Feature Vector (17x3 abs kpts, 17x2 relative, center/velocity/accel, 6 ball reserved, 28 court), Dataset, Fixed slicing, used for test set or initialization., Called before the start of each epoch to randomly re-slice training samples., TennisActionDataset, action_annotator.py — Flask action temporal annotation web app, add_court_to_pose.py — adds 14 court points into pose_data.json court field, batch_eval_all.py — batch-evaluates all trained models (+15 more)

### Community 8 - "Court Detection & Homography"
Cohesion: 0.10
Nodes (15): config_legacy.py — Batch Processing Pipeline Configuration (for use by main.py)…, CourtDetector, get_weighted_homography(), HomographyFilter, project_far_half_pixel_bbox(), court_detector.py — Court Detector (for use by main.py) Function: Detects the…, model: a pre-loaded YOLO court-keypoint model (see config.COURT_MODEL_PATH).…, Forces the homography state back to 'nothing detected yet'. Call this at a… (+7 more)

### Community 9 - "Demo Inference Thread"
Cohesion: 0.13
Nodes (17): _build_gt_labels(), _crop_fixed(), _draw_person(), _emit_result(), _nullctx, inference.py — Inference thread: real-time person YOLO + pose YOLO detection…, Constructs tensors, runs MSTFormer inference, and emits result signals., Draws the bounding box and skeletal keypoints on the frame. (+9 more)

### Community 10 - "Demo Main Window (PyQt5)"
Cohesion: 0.11
Nodes (4): QMainWindow, MainWindow, main(), main.py — Demo Entry Point

### Community 11 - "Batch Rally Pipeline"
Cohesion: 0.14
Nodes (13): BatchTennisPipeline, concat_videos(), Concatenates a list of video files (stream-copy) into one file using FFmpeg's…, Slices playing blocks into individual clips and compiles them into one final…, Lazily loads (once) the pose model used for annotating cut rally clips., Lazily loads (once) a second, independent pose-model instance used for the…, Lazily loads (once) the court keypoint model used by CourtDetector to fit the…, Renders a pose-annotated version of each cut rally clip, plus a combined… (+5 more)

### Community 12 - "Pose Tracker (Player Tracking)"
Cohesion: 0.13
Nodes (10): PoseTracker, Ankles (COCO indices 15/16), falling back to bbox bottom-center when they…, Supplemental far-half detection pass: runs a second, plain (untracked) YOLO-…, Greedy nearest-position association for the far-crop pass's pseudo-tracks.…, Merges tracks -- both full-frame BoT-SORT ints and far-crop "crop_N" pseudo-…, Picks the far-side and near-side track IDs. Tracks are first stitched together…, Pre-computes a frame_idx -> {"bbox", "keypoints"} dict for one selected track,…, Draws one already-smoothed render entry (see build_render_track) onto a frame. (+2 more)

### Community 13 - "Project Docs & Tech Stack"
Cohesion: 0.22
Nodes (19): CLAUDE.md — Project Guidance for Claude Code, annotations.json format (action time-segment labels), MSTFormer (Multi-Stream Transformer) — architectural concept, pose_data.json format (court + near/far player keypoints per frame), docs/architecture_zh.md — Chinese Architecture Doc (referenced, not read this chunk), docs/style_guide.md — Code Style Guide, FFmpeg (audio extraction, clip cutting/concatenation), Flask (web-based action annotator) (+11 more)
...
### Community 18 - "Trajectory Generation & Radar"
Cohesion: 0.18
Nodes (10): get_weighted_homography(), HomographyFilter, main(), RadarDrawer, Sliding average of trajectory points, Calculate the residual function of weighted projection errors, Use the weighted L-M algorithm to compute the precise homography matrix, First-layer filtering: Sliding window smoother for the court homography matrix H (+2 more)
...
### Community 20 - "Forehand Frame Extraction"
Cohesion: 0.20
...
### Community 21 - "Source Package Overview"
Cohesion: 0.20
Nodes (14): configs/person_sorter_dataset.yaml — Person Classifier Dataset, data/person_sorter/ — Person Classifier Dataset (near/far), src/model/mst/ package — MSTFormer Model, Training & Evaluation, src/model/mst/README.md — MSTFormer Guide (referenced, not read this chunk), src/model/yolo/ package — Single-Frame YOLO Baseline, src/model/yolo/README.md — Single-Frame YOLO Baseline Guide (referenced, not read this chunk), src/pipeline/ package — Offline Precision Tracking & Court Annotation Tools, src/pipeline/README.md — Offline Tracking & Data Tools Guide (referenced, not read this chunk) (+6 more)

### Community 23 - "Model Report Generation"
Cohesion: 0.15
Nodes (13): collect_all_data(), compute_confusion_matrix(), gen_confusion_matrix_html(), generate_report(), parse_csv(), plot_curves(), Reads all training outputs under models/action/ and generates a comprehensive…, Collect data for all models (+5 more)
...
### Community 28 - "Confusion Matrix Generation"
Cohesion: 0.22
Nodes (12): compute_cm(), find_model_dirs(), generate_report(), plot_cm(), For each fully trained model, run sample-by-sample inference on the test set to…, Compute the exact confusion matrix, Plot the confusion matrix, Run inference, collecting all predictions and labels (+4 more)
...
### Community 34 - "Hard Negative Frame Extractor"
Cohesion: 0.23
...
### Community 39 - "MST Config & Dataset Tests"
Cohesion: 0.31
Nodes (7): load_config(), config.py — YAML Configuration Parser, test_dataset.py — Dataset loading validation script, test_dataset_pipeline(), evaluate_and_plot(), test_matrix.py — Confusion Matrix Evaluation Script, split_dataset()
...
### Community 41 - "Person Detector Evaluation"
Cohesion: 0.27
...
### Community 44 - "Court/YOLO Training Entry Points"
Cohesion: 0.25
Nodes (6): export_bad_cases(), train_court_pipeline.py — Tennis Court Keypoints Model Training Entry Point…, train_model(), main(), yolo-train.py — YOLO Person Classification Model Training Script Function:…, YOLO

### Community 45 - "Broadcast Rally Detection"
Cohesion: 0.28
...
### Community 46 - "Confusion Matrix Evaluation (MST)"
Cohesion: 0.29
Nodes (4): 5-Class Action Taxonomy (idle/forehand/backhand/serve/movement), MSTFormer Action Classification Head, Fig. 7 — Column-Normalized Confusion Matrix (Precision View), Evaluate MSTFormer model and generate confusion matrices.

### Community 47 - "Demo Inference Thread (Player)"
Cohesion: 0.25
Nodes (4): QThread, InferenceThread, _get_short_path(), player.py — Video player based on QTimer + OpenCV frame-by-frame loading.
...
### Community 54 - "Sequence Length Sweep"
Cohesion: 0.38
Nodes (4): evaluate(), main(), _nullctx, seq_len_sweep.py — Sequence Length Sweep: Fixed weights, evaluate accuracy…
...
### Community 61 - "Batch Model Evaluation"
Cohesion: 0.50
Nodes (4): main(), batch_eval_all.py — Batch Evaluation Script: Single Process + Data Sharing (3…, Attempts to load a model weight checkpoint and configuration file. Returns None…, try_load()
...
- **What is the exact relationship between `configs/CONFIG_REFERENCE.md` and `MSTFormer Hyperparameter Sensitivity Finding (depth=8, dim=256, vt=16 near-optimal)`?**
  _Edge tagged AMBIGUOUS (relation: conceptually_related_to) - confidence is low._
- **Why does `docs/architecture.md — Project File Manifest & Module Dependencies` connect `Data Pipeline & Ball Tracking Docs` to `MSTFormer Model Architecture`, `Ablation & Component Configs`, `Ablation Results Mapping`, `YOLO Single-Frame Dataset`, `Pose Feature Dataset Tools`, `Court Detection & Homography`, `Pose Tracker (Player Tracking)`, `Project Docs & Tech Stack`, `Source Package Overview`?**
  _High betweenness centrality (0.194) - this node is a cross-community bridge._
- **Why does `MSTFormer` connect `Ablation & Component Configs` to `MSTFormer Model Architecture`, `Ablation Results Mapping`, `Config Reference & Figures`, `Data Pipeline & Ball Tracking Docs`, `YOLO Single-Frame Dataset`, `MST Config & Dataset Tests`, `Demo Inference Thread`, `Project Docs & Tech Stack`, `Confusion Matrix Evaluation (MST)`, `Demo Inference Thread (Player)`, `Source Package Overview`, `Sequence Length Sweep`, `Confusion Matrix Generation`, `Batch Model Evaluation`?**
  _High betweenness centrality (0.157) - this node is a cross-community bridge._
- **Why does `InferenceThread` connect `Demo Inference Thread (Player)` to `Demo Inference Thread`, `Demo Main Window (PyQt5)`, `Demo App Preferences`, `Ablation & Component Configs`?**
