...
| `court_detector.py` | Court homography detector: detects the 14 standard court keypoints with a YOLO keypoint model and fits a weighted, temporally-smoothed pixel↔real-world (meters) homography (RANSAC + Levenberg-Marquardt); same approach as `pipeline/offline_tennis_tracker.py` | Called by `main.py` |
| `pose_tracker.py` | Pose tracker: full-frame BoT-SORT + YOLO-pose, projected into real-world court coordinates via `court_detector.py`'s homography (plus a supplemental far-half crop pass and cross-source track stitching); selects near/far players by baseline proximity over the clip, includes EMA smoothing and dropped-frame compensation | Called by `main.py` |
